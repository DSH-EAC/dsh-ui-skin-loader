/**
 * VERBATIM vendored client module of the 交易终端（trading）skin.
 *
 * Source: DSH-EAC/DSH-Desktop-EAC @ 26841f5ee83c154a9768cc0a9cec1d70078f0ddf
 *         path dsh-desktop/assets/skins/trading/lib/client.js
 *         (upstream bundle of zhu1090093659/dsh-web-ui @linxin666/dsh-client-ui-skin-trading,
 *         BSD-3-Clause, Copyright (c) 2026, zhu1090093659 — full license text
 *         and provenance in ../../THIRD-PARTY-NOTICES.md).
 *
 * Covenant-conversion edits — the ONLY changes to the upstream bundle bytes;
 * everything else (esbuild region markers, original comments, logic, class
 * names, artwork data URIs) is byte-identical to the upstream artifact:
 *   1. de-shelled: removed the DSH module-loader factory shell (the
 *      `load({ id, factory })` wrapper this repo's build scripts emit around
 *      client bundles) and the `exports.apply = apply` glue (upstream bundling
 *      machinery, not skin content) and the dangling source-map reference
 *      comment at the tail (no map is shipped);
 *   2. S3/R1 (covenant conversion): the upstream module scope executed a
 *      style-tag injection at import time — "runs before activation" code.
 *      That block was relocated verbatim into the top of apply(), so importing
 *      this module now has zero side effects (R1: 未激活零副作用);
 *   3. `export { apply };` appended (ESM export of the original entry).
 */
