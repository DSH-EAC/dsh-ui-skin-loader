...
test("skin id matches the covenant §3 pattern and metadata is complete", () => {
  ...
  assert.equal(SKIN_META.id, "dsh-eac.skin.whale-song");
  assert.equal(SKIN_META.name, "鲸吟");
  assert.equal(ENTRY_ID, "dsh-eac-skin-whale-song");
});
...
test("whale-song preview SVG is deterministic and namespaced", () => {
  const first = WHALE_SONG_PREVIEW_SVG;
  assert.equal(first, WHALE_SONG_PREVIEW_SVG, "preview generation must be deterministic");
  ...
  assert.ok(!/\sid="(?!skn-whale-song)[^"]*"/.test(first), "no un-namespaced id attribute");
...
test("upstream body marker does not collide with the loader or other skins", () => {
  ...
  assert.ok(
    !["data-dsh-aurora", "data-dsh-inkwash", "data-dsh-trading", "data-dsh-dragon-heir"].includes(ACTIVE_BODY_MARKER),
    "not another skin's marker (R3)",
  );
  assert.equal(UPSTREAM_PACKAGE, "@linxin666/dsh-client-ui-skin-whale-song");
});
