...
import { DRAGON_HEIR_PREVIEW_SVG } from "./preview.ts";
...
test("skin id matches the covenant §3 pattern and metadata is complete", () => {
  ...
  assert.equal(SKIN_META.id, "dsh-eac.skin.dragon-heir");
  assert.equal(SKIN_META.name, "龙的传人");
  assert.equal(ENTRY_ID, "dsh-eac-skin-dragon-heir");
});
...
test("dragon-heir preview SVG is deterministic and namespaced", () => {
  const first = DRAGON_HEIR_PREVIEW_SVG;
  assert.equal(first, DRAGON_HEIR_PREVIEW_SVG, "preview generation must be deterministic");
  ...
  assert.ok(!/\sid="(?!skn-dragon-heir)[^"]*"/.test(first), "no un-namespaced id attribute");
...
test("vendored module keeps the upstream skin namespace and drops the bundling shell", () => {
  ...
  assert.ok(vendoredSource.includes("data:image/webp"), "the two theme artworks travel with the bundle");
  assert.ok(vendoredSource.includes("data:image/svg+xml"), "the seal favicons travel with the bundle");
...
test("upstream body marker does not collide with the loader or other skins", () => {
  ...
  assert.ok(!["data-dsh-aurora", "data-dsh-inkwash", "data-dsh-trading"].includes(ACTIVE_BODY_MARKER), "not another skin's marker (R3)");
  assert.equal(UPSTREAM_PACKAGE, "@linxin666/dsh-client-ui-skin-dragon-heir");
});
