/**
 * 龙的传人的激活会话（公约 §4.2 语义事件的皮肤侧实现）。
 *
 * 迁移形态：观感与行为内容 = `../vendor/dsh-web-ui-client.js` 的上游 apply
 * （原样迁移：亮暗双画背景层、主题跟随的遮罩/滤镜、龙印 favicon、样式注入），
 * 本模块只做公约化适配——把上游 cordis 形态的副作用收进 activate：
  ...
  function teardown(): void {
    ...
    for (let i = disposers.length - 1; i >= 0; i--) {
      try {
        ...
        skinCtx.logger.warn("dragon-heir: teardown step failed", { error: String(error) });
      }
    }
    skinCtx.logger.info("dragon-heir: deactivated — all side effects unwound");
  }
...
export function createDragonHeirActivation(ctx: SkinClientContext) {
  ...
  return {
    activate(skinCtx: SkinActivationContext): void {
      ...
      activeSession = activateDragonHeirSession(ctx, skinCtx);
      if (!safetyNetRegistered) {
        safetyNetRegistered = true;
        ctx.effect(() => () => activeSession?.teardown(), "skn-dragon-heir: session safety net (fiber dispose, R8)");
      }
    },
    deactivate(): void {
